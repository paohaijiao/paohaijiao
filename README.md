...
|---------|-------------|-------------|--------------|-------|---------------------------|
| [jquick-hub](https://github.com/paohaijiao/jquick-hub) | jquick-hub (TCP Gateway) Available as a Docker image: docker pull **paohaijiao/jquick-hub:latest**| A high-performance protocol-sniffing TCP gateway built on Netty 4. It exposes a single public port and automatically routes inbound connections to different backends based on the first packet | ❌ | [![Stars](https://img.shields.io/github/stars/paohaijiao/jquick-hub?style=flat-square&label=stars)](https://github.com/paohaijiao/jquick-hub) | No |
| [jquick-json](https://github.com/paohaijiao/jquick-json) | using **ASM** bytecode to generate alternative reflections, achieving a performance improvement of up to 54 times for JSON serialization/deserialization.| JQuick JSON is a component of the infrastructure layer in the jquick toolchain, which, in conjunction with **jquick-asm**, provides high-performance object serialization/deserialization capabilities for the entire toolchain. It is particularly suitable for scenarios that require high performance in JSON processing and want to keep the code concise. | ❌ | [![Stars](https://img.shields.io/github/stars/paohaijiao/jquick-json?style=flat-square&label=stars)](https://github.com/paohaijiao/jquick-json) | No |
...
| [jquick-excel](https://github.com/paohaijiao/jquick-excel) | Zero-config Excel read/write tool — say goodbye to POI boilerplate, import/export in one line | JQuick Excel solves large file OOM through **SXSSF streaming processing** (reducing 75% of 100000 lines of memory), breaks the Excel 64000 style limit for style **caching** (900000 to 10 copies), **eliminates duplicate expression** overhead for **AST** caching (200000 to 2 parsing), optimizes multi sheet reading for **OPCPackage** shared parsing (reducing import memory by over 50%), and achieves stable and efficient import and export of large amounts of Excel data through a combination of four underlying optimizations. | ✅ | [![Stars](https://img.shields.io/github/stars/paohaijiao/jquick-excel?style=flat-square&label=stars)](https://github.com/paohaijiao/jquick-excel) | Yes |
